...
2. PASSO A PASSO — RODAR EM QUALQUER MÁQUINA

   Setup completo para subir o projeto do zero em outro computador:
...
6. TESTES
Total: 131 testes, 26 suites, todos passando ✅

Backend (17 suites, 95 testes unitários)
Arquivo	Qtd	Cobertura
services/service.spec.ts	14	create, findAll, findOne, NotFound, update, remove
clients/service.spec.ts	14	create, findAll, findOne, NotFound, update, remove
appointments/service.spec.ts	17	create, NotFound x2, Conflict, findAll, findByDate, findOne, remove
availability/service.spec.ts	7	CRUD + geração de slots
public/service.spec.ts	3	getProfile, getServices, getSlots, createAppointment
auth/ service.spec.ts	14	register, login, forgot-password, reset-password, validações
controllers (x6)	26	smoke tests + responses HTTP
E2E (integração real com banco, contado acima)
- Fluxo completo: register → login → CRUD appointments/clients/services/public

Frontend (9 suites, 36 testes)
Arquivo	Qtd	Cobertura
lib/utils.test.ts	2	cn() merge de classes
lib/api.test.ts	6	ApiClient fetch, token, error handling
contexts/auth-context.test.tsx	8	login, logout, loading, protected state
components/ui/*.test.tsx	14	Button, Input, Card, Label, Table
middleware.test.ts	6	proteção /dashboard, redirect /login, /register

Para rodar:
  cd agendamento-backend && npm test        # Backend
  cd agendamento-frontend && npm test       # Frontend
  npm run test:e2e                           # E2E (requer PostgreSQL rodando)
7. DEPLOYMENT (Docker)
...
- Fase 6:
  ...
  * Campo viewed no Appointment para controle de notificação
   * Notificação via WhatsApp — ao confirmar agendamento na página pública, abre WhatsApp da maquiadora automaticamente com dados do cliente, serviço, data/horário e valor
   - Fase 7: Expansão de Testes (131 testes)
     * Backend expandido de 27 para 95 testes unitários (17 suites)
     * Frontend: 36 testes criados do zero (9 suites)
     * Cobertura: controllers, services, componentes UI, middleware, auth context
     * Plano de deploy zero-custo documentado (plano-deploy-zero-custo.md)
...
/agende/[slug]              /login → /dashboard
    ...
    (sem auth)                   /services, /clients (JWT)
                                  └── /auth/forgot-password
                                      /auth/reset-password
13. PLANO DE DEPLOY — CUSTO ZERO INICIAL
Ver arquivo completo: `plano-deploy-zero-custo.md`

Stack gratuita:
| Camada | Serviço | Cartão? |
|---|---|---|
...
| Email | Resend Free | ❌ |
Resumo dos passos:
1) Criar conta no Supabase → pegar DATABASE_URL
2) Adaptar código: migrar uploads (multer → Supabase Storage), integrar Resend no forgot-password, atualizar CORS
3) Deploy do backend no Render → Web Service → Docker
4) Deploy do frontend no Vercel → importar projeto → setar NEXT_PUBLIC_API_URL
5) Rodar npx prisma db push contra o Supabase
6) Configurar cron de keep-alive (cron-job.org ou UptimeRobot) para evitar cold start
7) Testar fluxo completo contra produção